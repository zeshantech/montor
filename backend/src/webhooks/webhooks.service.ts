...
import { BuildStatus } from '../cicd/cicd.entity';
...
export class WebhooksService {
  ...
  async handleGitHubWebhook(headers: any, body: any): Promise<void> {
    ...
    const project = await this.projectService.findOneByFilters({
    ...
    // Process the event
    this.logger.log(`Received event ${event} for project ${project.name}`);

    // Handle specific events
    switch (event) {
      case 'push':
        await this.handlePushEvent(project.id, body);
        break;
      case 'pull_request':
        await this.handlePullRequestEvent(project.id, body);
        break;
  ...
  // Handle Push Events
  async handlePushEvent(projectId: string, body: any): Promise<void> {
  ...
  async handlePullRequestEvent(projectId: string, body: any): Promise<void> {
